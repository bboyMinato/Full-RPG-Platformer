...
{
	TextureManager::GetInstance()->LoadTexture("Options Menu Background", "assets\\OptionsMenuBackground.jpg");
}
...
void OptionsState::HandleEvents()
{
	...
	if (Input::GetInstance()->GetKeyDown(SDL_SCANCODE_Q) && Input::GetInstance()->GetKeyDown(SDL_SCANCODE_SPACE))
		_data->_stateManager.AddState(CGameStateRef(new MainMenuState(_data)), true);
}
...
void OptionsState::Render(float dt)
{
	...
	TextureManager::GetInstance()->Draw("Options Menu Background", 0, 0, 1920, 1080);
